...
keywords_dictionary = {
    ...
    'progress_and_roadblocks': ['progress and roadblocks', 'progress and roadblock', 'progress &amp; roadblocks', 'Progress/Roadblocks', 'progress and challenges'],
    'progress': ['progress'],
...
def checkin_categories_to_df_1row(df, row_entry, text, indices_start_of_category, category_names, answers):
    project_counter = 0
...
def parse_1row(df):
    ...
    for i in range(len(df)):
        ...
        if len(indices_start_of_category) == 0:
            ...
            parsed_df = checkin_categories_to_df_1row(parsed_df, i, text, indices_start_of_category, category_names, answers)
...
def checkin_categories_to_df_nrows(df, text, indices_start_of_category, category_names, answers):
    """
    ...
    for i in range(len(df)):
        ...
        if i in sample_msg_indices:
            ...
            if projects_parsed == 0:
                ...
                df_i_blocks = checkin_categories_to_df_nrows(df_i_blocks, text, indices_start_of_category, category_names, answers)
