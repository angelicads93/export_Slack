...
from inputs import missing_value, timmeshift, chosen_channel_name, write_all_channels_info, write_all_users_info, slackexport_folder_path, converted_directory
import excel
import clean
import checkins
...
class InspectSource:
    ...
    def check_save_path_exists(self, path):
        if exists(path) is True:
            ...
            if exprt_folder_path.is_dir():
                print(f"The path '{path.split('JSONs')[0][:-1]}' already exists and it will be overwritten.")
    ...
    def get_channels_names(self):     # AG20241120
        ...
        if analyze_all_channels is False:
            if exists(f"{slackexport_folder_path}/{chosen_channel_name}") is False:
                self.channels_names = []
...
class SlackChannelsAndUsers:
    ...
    def get_all_users_info(self):
        ...
        for feature in ['display_name', 'name', 'team_id', 'id', 'profile_title', 'profile_real_name']:
...
class SlackMessages:
    ...
    def slack_json_to_dataframe(self, slack_json):
        ...
        for message in range(len(slack_json)):
            ...
            elif 'subtype' in slack_json[message]:
                messages_df.at[message, "msg_id"] = slack_json[message]['subtype']
            else:
                messages_df.at[message, "msg_id"] = missing_value
            ...
            if 'parent_user_id' in slack_json[message]:
                ...
                messages_df.at[message, "type"] = "thread"
            else:
                messages_df.at[message, "ts_thread"] = missing_value

            messages_df["text"] = messages_df["text"].astype(str)

            features = ['ts', 'user',  'text', 'reply_count', 'reply_users_count',  'parent_user_id']
    ...
    def add_users_info_to_messages(self, df_messages, df_users):
        ...
        for index in df_messages.index.values:
            ...
            if i_df['display_name'].shape[0] == 0 and df_messages.at[index, 'user'] == 'USLACKBOT':
    ...
    def ts_to_tz(self, df, original_column_name, new_column_name):
        ...
        df[original_column_name] = pd.to_numeric(df[original_column_name], errors='coerce')
        tzs = []
        for i in range(len(df)):
            i_is_null = pd.Series(df.at[i, original_column_name]).isnull().values[0]
    ...
    def get_all_messages_df(self):
        if self.continue_analysis is False:
            ...
            for i_channel in range(len(self.channels_names)):
                ...
                self.parent_user_id_to_name(channel_messages_df, channel_users_df)
